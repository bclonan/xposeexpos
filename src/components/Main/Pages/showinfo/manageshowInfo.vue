                ...
                <div class="navigation-tabs layout-tabs animated-tabs mt-20 mb-20">
                  ...
                  <div class="navtab-content" :class="[ isTabActiveNow === 'vendorList' ? 'is-active' : '']">
                    ...
                    <div class="projects-list-wrapper">
                      <div class="list-header">
                        <div class="list-title">
                          <span>Exhibitor Requests</span>
                        </div>
                      </div>
                      <div class="list-body">
                        <div class="columns is-multiline">
                          <approval-table @approveExhibitor="addExhibitor" @denyExhibitor="denyExhibitor" :organizerInfo="organizerInfo" />
                        </div>
                      </div>
                    </div>
                  </div>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      saveThePage() {
        ...
        var docData = {
          ...
          page_owner_id: this.currentUser.user_id,
      ...
      fetchExpoData() {
        ...
        expoRef
          ...
          .then(res => {
            ...
            this.current_page_id = res.data().expo_page_id;
      ...
      addExhibitor(i) {
        console.log(i);
        const exuserid = i.exhibitor_user_id;
        const usermessageId = i.exhibitor_message_id;
        let expo_id = this.$route.params.id;
        let vendor_id = i.id;
        let buzname = i.exhibitor_business_name;
        let exhibitorname = i.exhibitor_name;
        let exhibitor_request_id = i.request_id;

        //let expoApprovalRef = fb.expoApproval.doc(expo_id).collection('requests').doc();

        let expo_pg_id = uuid.v4();
        //ref to new team doc
        var expocollection = fb.expoCollection
          .doc(expo_id)
          .collection('vendors')
          .doc();
        //analytic ref
        ...
        batch.set(expocollection, {
          expo_id: expo_id,
          vendor_id: vendor_id,
          expo_owner_id: exuserid,
          expo_contact_name: exhibitorname,
          expo_owner_businessname: buzname,
          expo_description: '',
        ...
        return batch
          .commit()
          .then(() => {
            let expoApprovalRef = fb.expoApproval
              .doc(expo_id)
              .collection('requests')
              .doc(exhibitor_request_id);
      ...
      denyExhibitor(i) {
        console.log(i);
        let expo_id = this.$route.params.id;
        let exhibitor_request_id = i.req_id;
    ...
    computed: {
      ...
      organizerInfo() {
        let prof = this.$store.getters.getUserProf;
        const sendinfo = { messageBackID: prof.user_message_id, name: prof.Name };
        return sendinfo;
      },
